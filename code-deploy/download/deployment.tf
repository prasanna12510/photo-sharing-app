...
data "null_data_source" "lambda_file" {
  inputs = {
    filename = "${path.module}/src/download_image.py"
  }
}

data "null_data_source" "lambda_archive" {
  inputs = {
    filename = "${path.module}/src/download_image.zip"
  }
}

data "archive_file" "download_image" {
  type        = "zip"
...
module "download_image_lambda" {
  ...
  source_code_hash       = base64sha256(data.archive_file.download_image[0].output_path)
...
module "download_image_lambda_permission" {
  ...
  function_name = module.download_image_lambda.func_name
...
module "download_image_api_method" {
  ...
  http_method        = "GET"
  lambda_fuction_arn = module.download_image_lambda.arn
